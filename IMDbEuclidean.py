...
def cul_distance(centroid_tensor, centroid_tensor_lable, train_set, batch_num=4000):
    ...
    return tc.topk(tc.cat((a / a0.shape[0], b / b0.shape[0]), dim=0), dim=0, k=1, largest=False)[1]
...
if pic_num == 'all':
    lable_predict = tc.squeeze(cul_distance(b, train_labels, c))
    ...
    lable_predict = tc.squeeze(cul_distance(centroid_tensor, centroid_tensor_label, c))
